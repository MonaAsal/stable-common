...
class EditTextTimePicker: EditTextUniqueLight {
    private val POSITION_NONE: Int = -1
    ...
    override fun onFocusChanged(focused: Boolean, direction: Int, previouslyFocusedRect: Rect?) {
        ...
        currentPosition = if (focused) 0 else  -1
        updateText()
    ...
    private fun updateText() {
        ...
        val text: Spannable = SpannableString(String.format("%02d:%02d", getHour(), getMinutes()))
        if (bold >= 0) {
            text.setSpan(
    ...
    private fun onKeyEvent(keyCode: Int, event: KeyEvent?): Boolean {
        ...
        if (c >= '0' && c <= '9') {
            currentPosition =
                if (currentPosition ==  -1) 0 else currentPosition
            val n = c.code - '0'.code
            var valid = false
            ...
            if (valid) {
                ...
                currentPosition =
                    if (currentPosition < 3) currentPosition + 1 else  -1 // if it is the last digit, hide cursor
                updateText()
            }
